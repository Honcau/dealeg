...
import { VietQRTool } from '@/components/tools/VietQRTool';
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: 'tools.vietqr' });
  return { title: `${t('name')} | Dealeg`, description: t('desc') };
}

export default async function Page() {
  const t = await getTranslations('tools.vietqr');
  return (
