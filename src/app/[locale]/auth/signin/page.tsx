...
const GitHubIcon = () => (
  <svg className="w-5 h-5" viewBox="0 0 24 24" fill="currentColor">
    <path d="M12 0C5.374 0 0 5.373 0 12c0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23A11.509 11.509 0 0 1 12 5.803c1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576C20.566 21.797 24 17.3 24 12z"/>
  </svg>
);
...
function SignInForm() {
  ...
  async function handleSignIn(e: React.FormEvent) {
    ...
    if (res?.error) {
      setErrors({ form: t('wrongCredentials') });
      setLoading(null);
    } else {
  ...
  const socialBtn = 'flex items-center justify-center gap-3 w-full py-2.5 rounded-xl border font-medium text-sm transition-all hover:bg-gray-50 active:scale-95 disabled:opacity-50';
    ...
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 to-white flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl border border-gray-200 shadow-sm w-full max-w-md p-8">
        ...
        <div className="space-y-2.5 mb-5">
          <button onClick={() => handleSocial('google')} disabled={!!loading} className={`${socialBtn} border-gray-300`}>
            <GoogleIcon /><span>{t('withGoogle')}</span>
          </button>
          <button onClick={() => handleSocial('facebook')} disabled={!!loading} className={`${socialBtn} border-[#1877F2] text-[#1877F2] hover:bg-blue-50`}>
            <FacebookIcon /><span>{t('withFacebook')}</span>
          </button>
          <button onClick={() => handleSocial('github')} disabled={!!loading} className={`${socialBtn} border-gray-800 text-gray-800 hover:bg-gray-50`}>
            <GitHubIcon /><span>{t('withGithub')}</span>
          </button>
        </div>
        ...
        {/* Form */}
        {errors.form && (
          <div className="mb-4 text-sm text-red-500 bg-red-50 border border-red-200 rounded-lg px-3 py-2">{errors.form}</div>
        )}
        <form onSubmit={mode === 'signin' ? handleSignIn : handleSignUp} className="space-y-3">
          {mode === 'signup' && (
            <div>
              <input value={name} onChange={e => setName(e.target.value)} placeholder={t('name')} className={input(!!errors.name)} />
              {errors.name && <p className="text-xs text-red-500 mt-1">{errors.name}</p>}
            </div>
          )}
          <div>
            <input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder={t('email')} className={input(!!errors.email)} />
            {errors.email && <p className="text-xs text-red-500 mt-1">{errors.email}</p>}
          </div>
          <div>
            <input type="password" value={password} onChange={e => setPassword(e.target.value)}
              placeholder={mode === 'signup' ? t('passwordMin') : t('password')} className={input(!!errors.password)} />
            {errors.password && <p className="text-xs text-red-500 mt-1">{errors.password}</p>}
          </div>
          {mode === 'signup' && (
            <div>
              <input type="password" value={confirm} onChange={e => setConfirm(e.target.value)} placeholder={t('confirmPassword')} className={input(!!errors.confirm)} />
          ...
          <button type="submit" disabled={!!loading}
            ...
            {loading ? t('authenticating') : mode === 'signin' ? t('signIn') : t('createAccount')}
          </button>
        </form>
      </div>
    </div>
